...
html_file = open("ny_times_html_text.html", "w", encoding = "utf-8"	)
...
nytimes_headlines = []
first_soup = BeautifulSoup(html_text, 'html.parser')
#print(soup.prettify().encode('utf-8'))
for story_heading in first_soup.find_all(class_="story-heading"):
	if len(nytimes_headlines) < 10 and story_heading.a:
...
name_div = soup.find_all("div", {"class": "field-item even","property": "dc:title"})
#print(name_div)

for i in name_div:
	full_name = i.find("h2").text
	name.append(full_name)
#print(name)

title_div = soup.find_all(class_="field field-name-field-person-titles field-type-text field-label-hidden")
#print(title_div)

for j in title_div:
	full_title = j.find(class_="field-item even").text
	title.append(full_title)
#print(title)
for k in range(0,len(title)):
	umsi_titles[name[k]] = title[k]
#print(umsi_titles)
